...
ramp-blue:
	...
	-sed -e 's~<PROJECT_ID>~$(PROJECT_ID)~g' ./configs/kube/deployments-blue.yaml | kubectl apply -f -
ramp-green:
	kubectl apply -f ./configs/kube/services.yaml
	-sed -e 's~<PROJECT_ID>~$(PROJECT_ID)~g' ./configs/kube/deployments-green.yaml | kubectl apply -f -
route-green:
	-sed -e 's~<VCLUSTER>~green~g' ./configs/istio/routing.yaml | sed -e 's~<PROJECT_ID>~$(PROJECT_ID)~g'| kubectl apply -f -
route-blue:
	-sed -e 's~<VCLUSTER>~blue~g' ./configs/istio/routing.yaml | sed -e 's~<PROJECT_ID>~$(PROJECT_ID)~g'| kubectl apply -f -
